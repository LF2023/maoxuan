...
仓库名 **mxskill**，斜杠命令 **`/mxs`**。

把毛选里能复用的方法收成四行输出：待验证事实 → 主矛盾 → 放弃清单 → 结束条件。不是背语录。

**三条路（由快到慢）**

1. 微信搜一搜 **「问问教员」**，或扫下面的码  
2. Agent：`npx -y skills add LF2023/mxskill -g`，然后 `/mxs`  
3. [新手入门.md](./新手入门.md) · [English](./README_en.md)
...
```
/mxs 我现在又要抓私域、又要做投放、又要搞内容，忙不过来，怎么排优先级？
```

应得到四行：待验证事实 → 主矛盾假设 → 放弃清单 → 结束条件。不是平均待办清单。
...
完整内容见 [SKILL.md](./skills/mxs/SKILL.md)，触发场景索引见 [INDEX.md](./skills/mxs/INDEX.md)，触发 / 抗误触发测试见 [test-prompts.json](./test-prompts.json)。
...
>
> 示例输出形态：待验证事实 → 主矛盾假设 → 放弃清单 → 结束条件。完整对照见 [SKILL.md 示例输出](./skills/mxs/SKILL.md)。
